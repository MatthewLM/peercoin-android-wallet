...
package com.matthewmitchell.peercoin_android_wallet.integration.android;
 ...
 */
public final class PeercoinIntegration
{
	...
	private static final String MIMETYPE_PAYMENTREQUEST = "application/peercoin-paymentrequest"; // BIP 71

	/**
	 * Request any amount of Peercoin (probably a donation) from user, without feedback from the app.
	 * 
	 * @param context
	 *            Android context
	 * @param address
	...
	{
		final Intent intent = makePeercoinUriIntent(address, null);

		start(context, intent);
	}

	/**
	 * Request specific amount of Peercoin from user, without feedback from the app.
	 * 
	 * @param context
	 *            Android context
	 * @param address
	...
	/**
	 * Request any amount of Peercoin (probably a donation) from user, with feedback from the app. Result intent can be
	 ...
	 * 
	 * Warning: A success indication is no guarantee! To be on the safe side, you must drive your own Peercoin
	...
	/**
	 * Request specific amount of Peercoins from user, with feedback from the app. Result intent can be received by
	...
	/**
	 * Put BIP70 payment message into result intent. Meant for usage by Peercoin wallet applications.
	 * 
	 * @param result
	 *            result intent
	 * @param payment
	...
	/**
	 * Get BIP70 payment message from result intent. Meant for usage by applications initiating a Peercoin payment.
	 * 
	 * You can use the transactions contained in the payment to validate the payment. For this, you need your own
	 * Peercoin infrastructure though. There is no guarantee that the payment will ever confirm.
	 * 
	 * @param result
	 *            result intent
	...
	/**
	 * Put transaction hash into result intent. Meant for usage by Peercoin wallet applications.
	 * 
	 * @param result
	 *            result intent
	 * @param txHash
	...
	/**
	 * Get transaction hash from result intent. Meant for usage by applications initiating a Peercoin payment.
	 * 
	 * You can use this hash to request the transaction from the Peercoin network, in order to validate. For this, you
	 * need your own Peercoin infrastructure though. There is no guarantee that the transaction has ever been broadcasted
	 * to the Peercoin network.
	 * 
	 * @param result
	 *            result intent
	...
	{
		final StringBuilder uri = new StringBuilder("peercoin:");
		if (address != null)
			uri.append(address);
		if (amount != null)
